...
describe Iec60584 do
    ...
    [:b, :e, :j, :k, :n, :r, :s, :t].each do |kind|
      context "on a type #{kind} thermocouple" do
        ...
        it 'raises error when out of range' do
          expect { Iec60584.temperature(tc, examples[kind].first[:e] - 0.002) }.to raise_error(RangeError)
          expect { Iec60584.temperature(tc, examples[kind].last[:e]  + 0.002) }.to raise_error(RangeError)
        end
      end
    end
  end
  ...
  context 'emf computation' do
    # Add lower limit form emf equations, if they are lower than the lower limits for temperature equations
    examples[:b].unshift({ e:  0.000, t:     0.0 })
    examples[:e].unshift({ e: -9.835, t:  -270.0 })
    examples[:k].unshift({ e: -6.458, t:  -270.0 }) 
    examples[:n].unshift({ e: -4.345, t:  -270.0 })
    examples[:t].unshift({ e: -6.258, t:  -270.0 })
